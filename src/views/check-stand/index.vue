<template>
  <div class="check-stand-page">
    <div class="title">订单应付金额{{ seen }}</div>
    <div class="amount">
      ¥{{ formData && formData.payAmount | formaterMoney }}
    </div>
    <div class="tips">
    ...
    <div class="main-box">
      ...
      >
        <div class="wechat-l">
          <img
            class="icon-pay"
            src="@/assets/imgs/pay/icon-wechat-pay.png"
            alt=""
          />
      ...
      >
        <div class="alipay-l">
          <img
            class="icon-pay"
            src="@/assets/imgs/pay/icon-alipay.png"
            alt=""
          />
    ...
    <button
      ...
      @click="goPay"
      :disabled="payment === '0'"
      :style="{ opacity: payment !== '0' ? '1' : '0.5' }"
    >
      去支付
    </button>
    <button
      v-if="['weixin', 'alipay'].includes(mpType)"
      class="cancel-btn"
      @click="handleCancel"
    >
      放弃支付
...
export default {
  ...
  data() {
    return {
      ...
      iconCancel:
        "https://ggllstatic.hpgjzlinfo.com/static/supermarket/icon-index.png",
      // 显示支付弹窗
      show: false,
      // 运行环境
      phoneType: "",
  ...
  methods: {
    ...
    // 0元支付
    async hanldeOrderAmount(formData) {
      try {
        const result = await payApi.checkOrderAmount({
        ...
        if (result.data) {
          ...
          params.secenType = params.orderSource;
          this.resultSkipUrl(result.data, params);
        }
      } catch (err) {
        this.$toast(err.message);
      }
    ...
    // 处理0元支付结果
    async resultSkipUrl(result, paramsData) {
      switch (result) {
        case "SUCCESS":
          paramsData.type = 0;
    ...
    goOrderDetail() {
      ...
      if (this.phoneType === "alipay" || this.phoneType === "weixin") {
        wx.miniProgram.reLaunch({
          url: `/sub-pages/me/order-detail/main?id=${this.formData.transactionSerialNo}`,
        });
        window.my &&
          window.my.reLaunch({
            url: `/sub-pages/me/order-detail/main?id=${this.formData.transactionSerialNo}`,
    ...
    goBindCard() {
      wx.miniProgram.redirectTo({
        url:
          "/pages/pay/add-bank-card?source=0&cashId=" +
          this.cashId +
          "&secenType=" +
          this.secenType,
      });
    ...
    goHome() {
      switch (this.secenType) {
        case 0:
          // 附近优惠
          wx.miniProgram.nam({ url: "/pages/index/index" });
          window.my && window.my.reLaunch({ url: "/pages/index/index" });
          window.qa && window.qa.reLaunch({ url: "/pages/index/index" });
          break;
        case 5:
          wx.miniProgram.reLaunch({ url: "/pages/index/index?index=1" });
          window.my &&
            window.my.reLaunch({ url: "/pages/index/index?index=1" });
          window.qa &&
            window.qa.reLaunch({ url: "/pages/index/index?index=1" });
          break;
        case 9:
          // 商城
        ...
        default:
          ...
          window.my &&
            window.my.reLaunch({ url: "/pages/index/index?index=3" });
          window.qa &&
            window.qa.reLaunch({ url: "/pages/index/index?index=3" });
          break;
      }
    },
  },
